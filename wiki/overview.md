...
- PR #249 completed guided entry with the short default request `Analyse cette image avec Norma` and six visible workflow goals. PR #250 restored the fixed-array schema shape required by ChatGPT. The first preparation-only six-case run then exposed one real validation-boundary defect: segment and axis envelopes could disagree with their endpoint geometry. PR #251 made endpoints authoritative and derived the redundant envelope server-side. The exact-main corrective replay passed all four previously non-PASS cells with 25 candidates, zero retries, zero confirmations, zero Core runs, and no technical validation error. Exact main is `593b014052bc98e1065c2ce020f75f831f4a81d6`; the byte-identical private runtime is `live`/`ready` and passes direct MCP smoke after one bounded restart. Guided entry is complete; current routing is observation-led maintenance.
