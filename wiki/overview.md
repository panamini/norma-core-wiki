...
- PR #239 completed the altitude gate; PR #240 added the bounded preparation diagnostic; and PR #243 closed `CC-20260717-PERSONAL-MAIN-LIVE-ACCEPTANCE v6` with a live PASS while preserving `candidateEvidenceOnly=true`, `sourceTruth=false`, and `coreAuthority=false`. The held-out architecture/trapezoid, poster/refinement, and ambiguous-negative matrix later passed without reproducing a Core bug. PR #244 then added deterministic opt-in reports between exactly two declared confirmed lengths at merge `0fec3e7d55eecbed41ec75f38660742c15ce19dc`; PR #245 integrated the private-widget UI at the then-current exact main `b93dbb824617b5624803a4a847ee164b7b1d3bf7`. That historical runtime was byte-identical, passed build and suite `1707/1707`, reported `live`/`ready`, and the live ChatGPT gate preserved declared references through a quadrilateral edit, recovered from an incomplete toggle, and produced a separate `2/3` match with one confirmation/Core. The personal MVP is stable; further work is observation-led maintenance, not automatic geometric expansion. Circumcenter, incenter, orthocenter, other new centers, rhythms/repetitions, physical perspective/homography/rectification, and new harmonic-report families remain deferred.
- PR #246 added editable ellipse center/radius controls and responsive stacking. PR #247 kept off-frame ellipse editing reachable through bounded edge proxies without claiming unseen pixels. PR #248 added one removable pre-confirmation manual segment for a visually missed line or axis. At that checkpoint exact main was `c406e1c0b20178b6d97299b466d6c35893873183`; its byte-identical private runtime passed build and suite `1722/1722` and reported `live`/`ready`. The post-PR248 live acceptance proved manual creation, separate pixel refinement, explicit adopt/revert, complete deletion, reload restoration, one bounded confirmation/Core, and fresh same-file repréparation without inherited manual or completed state. Automatic visual proposal remains non-exhaustive; pixel refinement does not discover new primitives.
- Personal Visual Acceptance Pack v1 froze four synthetic cases and two real images before live execution. All `6/6` preparations were semantically acceptable, `5/6` completed one bounded confirmation/Core, and the matrix required zero human geometry corrections and zero accepted-operation retries in the same case/conversation. Case 2 used one case-level fresh isolated execution after a non-terminal surface attempt; no accepted preparation, confirmation/Core call, provider call, or recovery action was repeated in that same conversation. The remaining non-terminal confirmation was a ChatGPT surface block. The next observed need is guided entry through a short default prompt or visible goal presets, not new geometry or automatic ratio-pack discovery.
- PR #249 completed guided entry with the short default request `Analyse cette image avec Norma` and six visible workflow goals. PR #250 restored the fixed-array schema shape required by ChatGPT. The first preparation-only six-case run on pre-correction exact main `0981331e77c218c67eb6a9fbfbdec7fe396d9945` then exposed one real validation-boundary defect: segment and axis envelopes could disagree with their endpoint geometry. PR #251 made endpoints authoritative and derived the redundant envelope server-side. The exact-main corrective replay passed four fresh case-level reruns of the previously non-PASS cells with 25 candidates, zero accepted-operation retries inside any rerun conversation, zero confirmations, zero Core runs, and no technical validation error. Exact main is `593b014052bc98e1065c2ce020f75f831f4a81d6`; the byte-identical private runtime is `live`/`ready` and passes direct MCP smoke after one bounded restart. Guided entry is complete; current routing is observation-led maintenance.
- PR #274 is the latest merged state at `f84a71cb9e1d592eeadbc749cdb1f48e796d556b`; it closed the immediate-revocation review gaps. PR #258 remains the provider-neutral Railway → Supabase/RLS boundary and Scalekit-first MCP sandbox contract. The result is `observation-partial`: declared geometric comparisons are useful, while artistic usefulness, latency, mobile proof, sustained use, and commercial/public readiness remain unproven. The Railway/Supabase gate is `SHORTLISTED_PENDING_SANDBOX`: Scalekit first sandbox, Auth0 fallback, WorkOS conditional, and no production provider lock. No production provider, migration, or resource creation is authorized here. See `wiki/tech/compute-topology-optional-gpu-escape-hatch.md` for the local/CPU/optional-GPU boundary.
- The sandbox repository and disposable live path now pass Railway readiness, Supabase TLS, RLS allow/deny, missing-context deny, rollback isolation, pool reset, protected Scalekit business access, native OAuth/PKCE MCP transport, and immediate same-token revocation. Railway deployment `8fa03f94-595c-49fb-82e3-121aa2808ab0` revalidated exact merged `main@f84a71c` through `200 → 401 → 200`, with boolean cutoff/database-cleanup confirmation and no persisted token material. The preliminary Inspector DCR client was deleted in the Scalekit admin session and exact absence was verified (`count 0`); the retained proof client was untouched. The immediate-revocation sandbox rail is `CLOSED/DONE`. Production consent/refresh qualification remains a separate `CLOSED` decision and is not a blocker for this sandbox rail. For pack, performance, or UI/UX changes, run only narrow affected tests; reopen full auth/RLS qualification only for auth/OAuth/JWT/scope/audience/issuer/subject/tenant mapping, PostgreSQL/RLS/schema/pool reset, Railway deployment/config/runtime entrypoint, or provider metadata/registration changes. See `wiki/outputs/2026-07-25-scalekit-supabase-sandbox-qualification-checkpoint.md`.
...
- PR #238 merged exactly three opt-in internal angle bisectors for the same
...
- Current roadmap model is gate-based. Rotated-ellipse shadow refinement is
  ...
  separate live run, and PR #239 completed the altitude gate with exactly one
  explicit canonical triangle yielding exactly three derived altitudes, a
  ...
  and PR #244–#251 plus the declared-ratio and guided-entry live gates establish the current
