...
- The sandbox repository and disposable live path now pass Railway readiness, Supabase TLS, RLS allow/deny, missing-context deny, rollback isolation, pool reset, protected Scalekit business access, native OAuth/PKCE MCP transport, and immediate same-token revocation. PR #273 merged at `8037ddd211d71891a8f0f66cfa761091595f8684`; its hardened path passed a durable HMAC cutoff sequence `200 → 401 → 200` with verified proof-row cleanup. The qualification gate remains `CLOSED` until the remaining consent/refresh evidence and complete matrix are reviewed. See `wiki/outputs/2026-07-25-scalekit-supabase-sandbox-qualification-checkpoint.md`.
