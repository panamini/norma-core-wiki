...
- The sandbox repository and disposable live path now pass Railway readiness, Supabase TLS, RLS allow/deny, missing-context deny, rollback isolation, pool reset, protected Scalekit business access, and native OAuth/PKCE MCP transport. PR #273 candidate code also passed immediate same-token revocation with a durable HMAC cutoff: `200 → 401 → 200`, followed by verified proof-row cleanup. The qualification gate remains `CLOSED` until PR #273 merges and the remaining consent/refresh evidence and complete matrix are reviewed. See `wiki/outputs/2026-07-25-scalekit-supabase-sandbox-qualification-checkpoint.md`.
