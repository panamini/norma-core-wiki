...
- PR #281 and PR #282 added the authenticated, fail-closed SAM 3 Modal sandbox boundary and its packaged runtime. PR #283–#287 repaired the ChatGPT resource metadata, cache/MIME compatibility, stale aliases, and app-only remount path. PR #288 then completed the two-length selector consistency fix after a live recheck. Current `origin/main` is `ea12c967bbd7c3acfd8401e5d75dd6f59e8d07e4`. A fresh widget loaded with editable rectangle, ellipse, axis, and segment candidates after the resource repairs; no SAM receipt or mask timing was captured, so live SAM quality and latency are not claimed. The next bounded product slice is an explicit semantic SAM target using the existing text-prompt contract.
