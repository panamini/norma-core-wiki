...
- PR #258 merged the provider-neutral Railway → Supabase/RLS boundary and Scalekit-first MCP sandbox contract at `d6b3e6f8726a2dc3c2a476be6207e22339ccb9ca`; PR #257 merged the performance truth harness and PR #256 remains the latest visual-harmony product gate. The result is `observation-partial`: declared geometric comparisons are useful, while artistic usefulness, latency, mobile proof, sustained use, and commercial/public readiness remain unproven. The Railway/Supabase gate is `SHORTLISTED_PENDING_SANDBOX`: Scalekit first sandbox, Auth0 fallback, WorkOS conditional, and no production provider lock. No production provider, migration, or resource creation is authorized here. See `wiki/tech/compute-topology-optional-gpu-escape-hatch.md` for the local/CPU/optional-GPU boundary.
- The sandbox checkpoint is now complete on the repository and disposable live path: PR #272 merged the TLS/pool-reset/startup hardening; Railway deployment `e0da5150` served readiness 200; the deployed application factory verified Supabase TLS; live RLS allow/deny, missing-context deny, rollback isolation, pool reset, fixture cleanup, and one protected Scalekit business call passed. A native SDK smoke also passed OAuth/PKCE, `initialize`, `tools/list`, `resources/list`, and `resources/read`, observing four tools and one widget resource. The qualification gate remains `CLOSED` pending refresh/revocation and independently sanitized token-verification evidence. See `wiki/outputs/2026-07-25-scalekit-supabase-qualification-checkpoint.md`.
