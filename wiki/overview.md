...
- PR #252 separated dense overlay labels, PR #253 stabilized the explicit two-length comparison before confirmation, and PR #254 added correlated latency milestones without changing geometry or Core authority. PR #255 then hardened CI push-baseline selection only. Current exact main is `077671ed49e6807066e6138bec0b3556065bd725`; the exact built tree is promoted to the existing private runtime, which is `live`/`ready` and passes direct three-tool MCP smoke after one bounded restart. A fresh `Moyenne` acceptance on `DSC09546.jpeg` prepared 1 rectangle and 6 editable segments in about 58.9 s and completed one Core confirmation in about 7.1 s without human geometry edits. Usefulness is partial: the visible lengths are concrete, but the disabled two-length report produced no harmonic report or visual relation.
- A later fresh `Moyenne` conversation generated a synthetic two-bar image and accepted one 5-primitive Norma preparation. The axes measured about 475 px and 667 px (`0.712`), exposing that the raster did not preserve the requested `2:3` design. The widget remained blank for more than two minutes during the initial observation, then hydrated without a resend and reached `CORE VÉRIFIÉ`; the human two-length report was not selected. A fresh manually attached 860 x 854 geometric composition then prepared 6 rectangles and 2 axes in about 71 s and completed Core in about 12.6 s. Core returned three concrete rectangle reports, but the two-length report remained disabled with no selectable input despite the two visible axes. Across the spectacle and geometric-composition cases this is a reproduced availability gap for the opt-in comparison surface, not a Core failure. The smallest justified candidate change is to expose already-confirmed visible segment and axis guides to exactly two human-selected length inputs; geometry and harmonic-pack expansion remain frozen.
