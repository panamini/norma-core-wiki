...
- Sandbox checkpoint 2026-07-26: PR #272 is merged; Railway/Supabase TLS and RLS isolation, native OAuth/PKCE MCP transport, and immediate same-token revocation are PASS. PR #273 candidate code produced `200 → 401 → 200` around a durable HMAC cutoff with verified cleanup and no persisted token material. Production readiness remains `CLOSED` until PR #273 merges and the remaining consent/refresh evidence and complete matrix are reviewed.
...
- Immediate revocation is live-proven; merge PR #273 only after exact-head
  gates, then finish consent/refresh evidence and the complete matrix before
