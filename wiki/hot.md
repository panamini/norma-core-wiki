...
- Sandbox checkpoint 2026-07-26: Railway deployment `8fa03f94` serves exact merged `main@f84a71c`; Railway/Supabase TLS and RLS isolation, native OAuth/PKCE MCP transport, and immediate same-token revocation are PASS. The exact merged path reproduced `200 → 401 → 200` around a durable HMAC cutoff with verified database cleanup and no persisted token material. The preliminary Inspector DCR client was deleted in Scalekit and exact absence verified (`count 0`); the retained proof client was untouched. Immediate-revocation sandbox rail is `CLOSED/DONE`; production consent/refresh qualification remains a separate `CLOSED` decision. Pack, performance, and UI/UX changes need only narrow affected tests; reopen full auth/RLS qualification only for auth/RLS, Railway runtime, or provider metadata/registration boundary changes.
...
- Immediate revocation sandbox rail is complete; keep future production
  consent/refresh qualification separate and do not treat it as a blocker for
  this rail.
